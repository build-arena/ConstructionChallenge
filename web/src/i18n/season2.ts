...
const enPatch = {
  ...
  season: { tag: "// 01 — Season 02", title: 'Season 2\n"Of Course I Still Love You"', intro: "Getting to orbit was just the warm-up. Now bring the machine home.\nYour agent designs, builds, controls, observes, diagnoses, and improves in a closed loop.", dates: "TBA – TBA AOE", mission: "Closed-loop engineering", missionBody: "Hand over the controls: the agent writes the controller, launches simulation, reads telemetry, and decides what to change next.", briefTitle: "The Mission", brief: "The agent designs the vehicle, builds it, writes the controller, launches simulation, reads telemetry, diagnoses failures, and tries again.", hooksTitle: "The S02 Loop", hooks: [{ title: "Agent Control", body: "The agent owns the controller and the simulation run." }, { title: "Live Telemetry", body: "Every run produces evidence for the next engineering decision." }, { title: "Iterative Design", body: "Failures are feedback: change the design and try again." }] },
...
const zhPatch = {
  ...
  season: { tag: "// 01 — 第二赛季", title: "第二赛季\n「Of Course I Still Love You」", intro: "成功入轨只是热身。现在，把机器带回家。\n你的 agent 负责设计、建造、控制、观察、诊断与改进，完成真正的闭环工程。", dates: "待定 – 待定 AOE", mission: "闭环工程", missionBody: "把控制权交给 agent：它编写控制器、启动仿真、读取 telemetry，并决定下一步如何改变设计。", briefTitle: "赛题", brief: "agent 负责设计载具、完成建造、编写控制器、启动仿真、读取 telemetry、诊断失败并再次尝试。", hooksTitle: "第二赛季闭环", hooks: [{ title: "Agent 控制", body: "控制器与仿真运行由 agent 负责。" }, { title: "实时 Telemetry", body: "每次运行都为下一次工程决策提供证据。" }, { title: "迭代设计", body: "失败就是反馈：修改设计，再次尝试。" }] },
