...
const enPatch = {
  hero: {
    ...
    meta: "2026.09.20 – 2026.11.20 AOE · BuildArena × Besiege",
  },
  gameKey: {
    notice: "Besiege + The Broken Beyond DLC required · Steam key via the official form",
  ...
  tracks: {
    ...
    items: [
      { name: "Build with Agent", badge: "Main competitive", badgeVariant: "default", tagline: "Hand over the controls.\nDeliver. Separate. Return.", rules: ["Autopilot: no human engineering guidance after Engineering Start; score coefficient ×1.25.", "Copilot: natural-language collaboration is allowed; score coefficient ×1.00.", "The agent must design/build the machine, write the controller, launch the simulation, inspect telemetry, and iterate.", "Humans may not manually edit the machine/controller, directly tune engineering parameters, or pilot development/final flights."] },
      { name: "Human BOSS Challenge", badge: "Exhibition / baseline", badgeVariant: "accent", tagline: "Build, tune, and fly by hand.\nA separate human baseline.", rules: ["Build, tune, and fly the machine by hand under the same mission and physical restrictions.", "This track is separate from the main Build with Agent cash prizes.", "The highest-scoring hand-made machine is eligible for Best Human BOSS Build."] },
    ],
  },
  how: {
    tag: "// 03 — Mission Loop",
  ...
  scoring: {
    ...
    performance: [
      ...
      { name: "Payload Fuel Efficiency", weight: "15%", body: "η = M_payload / F_expended. Fuel left on lost hardware counts as expended; fuel brought home does not. A verified legal zero-propellant mission receives 15/15." },
      { name: "Recovery Completeness", weight: "30%", body: "30 × validly recovered dry hardware mass / initial non-payload dry hardware mass. Recovery requires ≥90% structural integrity and stable landing without propulsive support for 10 seconds." },
    ...
    cost: [{ name: "Three-orbit gate", body: "The payload must fly unpowered through three complete revolutions, with the entire payload strictly above 1100 game-distance units from the planet center during the qualifying interval; otherwise competitive score is 0." }, { name: "Fuel accounting", body: "Fuel expenditure covers the entire mission, including recovery. Fuel remaining in validly recovered hardware is not expended." }],
    ...
    tokenNote: "The official S02 scorer/reference implementation is part of the public BuildArena 2.0 release. Multiple submissions are allowed; one Effective Final Submission per team is rescored globally.",
  },
  submission: {
    ...
    humanDeliverables: ["human-built machine", "run record", "Kaggle writeup", "Best Human BOSS Build eligibility"],
    ...
    review: ["All writeups are due by Nov 20, 2026 AOE.", "Multiple submissions are allowed; the highest team submission becomes the Effective Final Submission.", "Scoring and winners follow unified verification after the deadline."],
  ...
  awards: {
    ...
    community: [{ name: "Community Favorite", prize: "$200" }, { name: "Best Human BOSS Build", prize: "Title" }, { name: "Multiple titles", prize: "Allowed" }, { name: "Major cash prizes", prize: "One per machine" }],
  ...
  leaderboard: { tag: "// 07 — Leaderboard", title: "Season 02 Leaderboard", status: "Not open yet", tabs: { overall: "Overall", autopilot: "Autopilot", copilot: "Copilot", human: "Human BOSS" }, columns: ["#", "Team", "Machine", "Mode", "Payload", "Efficiency", "Recovery", "Landing", "Performance", "Final", "Status"], emptyTitle: "The loop is warming up", emptyBody: "The competition opens Sep 20, 2026 AOE. Submit a verified final mission to appear after unified scoring.", provisional: "Provisional" },
  faq: {
    tag: "// 08 — Rules FAQ",
    title: "Season 02 FAQ",
...
const zhPatch = {
  ...
  tracks: { tag: "// 02 — 赛道与规则", title: "选择你的工程闭环", intro: "Build with Agent 是主赛道；Human BOSS 是独立的人类建造展示与基线赛道。", coefficientLabel: "模式系数", rulesLabel: "官方限制", note: "准备工程系统，而不是成品答案。禁止预制任务机器和可直接运行的任务控制器。", items: [{ name: "Build with Agent", badge: "主赛道", badgeVariant: "default", tagline: "把控制权交给 agent。\n交付、分离、返回。", rules: ["Autopilot：Engineering Start 后不得接受人类工程指导，模式系数 ×1.25。", "Copilot：允许自然语言协作，模式系数 ×1.00。", "agent 必须设计/建造机器、编写控制器、启动仿真、检查 telemetry 并迭代。", "人类不得手动编辑机器/控制器、直接调工程参数或驾驶开发/最终飞行。"] }, { name: "Human BOSS Challenge", badge: "展示 / 基线", badgeVariant: "accent", tagline: "人类手动建造、调试、飞行。\n独立的人类基线。", rules: ["在相同任务和物理限制下手动建造、调试和驾驶。", "与主赛道 Build with Agent 现金奖项分开。", "最高分手工机器可获得 Best Human BOSS Build。"] }] },
  how: { tag: "// 03 — 任务闭环", title: "建造 · 飞行 · 观察 · 改进", intro: "设计 → 建造 → 飞行 → 观察 → 诊断 → 改进 → 重复。", phases: [{ key: "prepare", label: "准备", steps: [{ n: "01", title: "准备系统", body: "获取 Besiege + The Broken Beyond，配置官方 BuildArena 2.0 MCP，定义 agent workflow。" }, { n: "02", title: "设计并建造", body: "由 agent 通过 BuildArena MCP 创建载具，禁止从成品任务机器或控制器开始。" }] }, { key: "compete", label: "运行任务", steps: [{ n: "03", title: "编写控制器", body: "agent 编写任务控制器、启动 Besiege、释放无动力载荷并操作回收硬件。" }, { n: "04", title: "观察并改进", body: "读取 telemetry、诊断失败、修改控制器或载具并运行下一次实验。" }] }, { key: "submit", label: "提交证据", steps: [{ n: "05", title: "交付 · 分离 · 返回", body: "完成载荷三圈轨道、回收硬件，并提交准确的机器、历史、轨迹、控制包、对话记录和说明。" }] }], gameKeyCta: "获取 Besiege + The Broken Beyond", submitBodyForm: "通过 Kaggle 提交完整的最终任务记录。", repoCta: "下载 BuildArena 2.0", cta: "前往 Kaggle 提交" },
  scoring: { tag: "// 04 — 官方评分", title: "交付 · 分离 · 返回", intro: "无动力载荷必须完成三圈轨道；之后表现分乘以建造模式系数。", performanceTitle: "PerformanceScore · 100 分", performance: [{ name: "轨道载荷", weight: "15%", body: "交付更多有效载荷质量。正向合格运行至少 6/15，适用池中最好成绩为 15/15。" }, { name: "载荷燃料效率", weight: "15%", body: "η = 载荷质量 / 消耗燃料。丢失硬件中的燃料计入消耗，带回的燃料不计入。合法零推进剂任务直接得 15/15。" }, { name: "回收完整度", weight: "30%", body: "30 × 有效回收干硬件质量 / 初始非载荷干硬件质量；需结构完整度 ≥90%，无推进稳定着陆 10 秒。" }, { name: "着陆精度", weight: "40%", body: "按质量加权的着陆精度；距离越近越高，超过阈值快速下降，未回收部分为零。" }], costTitle: "任务门槛与边界", cost: [{ name: "三圈门槛", body: "载荷须无动力完成三圈完整轨道，合格期间全程严格高于行星中心 1100 游戏距离单位，否则竞争得分为 0。" }, { name: "燃料记账", body: "整个任务（含回收）都计入燃料消耗；有效回收硬件中剩余燃料不计入。" }], formulaLabel: "官方公式", formulas: [{ label: "PerformanceScore", value: "PayloadScore + EfficiencyScore + RecoveryScore + LandingScore" }, { label: "FinalScore", value: "PerformanceScore × BuildModeCoefficient" }, { label: "模式系数", value: "Autopilot ×1.25 · Copilot ×1.00" }], finalTitle: "任务要求", finals: [{ label: "任务", value: "交付 · 分离 · 返回" }, { label: "合格载荷", value: "三圈完整轨道 · 无动力 · >1100 距离单位" }], tokenNote: "官方 S02 scorer/reference implementation 随 BuildArena 2.0 公共发布；允许多次提交，但每队只有一个 Effective Final Submission 进入全局重算。" },
  submission: { tag: "// 05 — 提交与校验", title: "提交完整记录", intro: "Build with Agent 通过 Kaggle 提交最终机器以及可复现的控制与证据包。", deliverablesTitle: "Build with Agent · 6 项必交", deliverables: ["machine.bsg — 最终评分机器", "build_history.json", "build_history_full.json", "trajectory.csv — 单次最终任务官方 telemetry", "control/ — 控制器源码、配置、说明和一键复现", "chat_transcript.md — Autopilot 或 Copilot 记录"], manifestTitle: "说明必须包含", manifestFields: ["Machine Name / Track / Autopilot 或 Copilot", "Mission Summary 与 Final Run 指标", "Video（鼓励提交）", "LLM / Agent Setup", "Prompts and Workflow", "Control System", "Engineering Iteration", "Notes"], humanTitle: "Human BOSS", humanDeliverables: ["人类建造机器", "运行记录", "Kaggle 说明", "Best Human BOSS Build 资格"], validationTitle: "第二季校验规则", validations: [{ q: "Build with Agent 禁止人工控制", a: "由 agent 驾驶提交机器；人类不得编辑几何/控制器、调工程参数或驾驶开发/最终飞行。" }, { q: "控制器可复现", a: "control/ 必须说明 telemetry 输入输出、阶段、状态、控制逻辑、分离、回收、着陆、阈值和任务完成；官方入口必须能复现运行。" }, { q: "证据完整", a: "历史记录必须对应 machine.bsg，trajectory.csv 必须来自 Evaluation Endpoint 的单次最终任务。" }, { q: "拒绝条件", a: "人工改几何/结构、改变行为的 mod、非法 vanilla 数值、伪造/篡改证据或使用禁止的成品机器/控制器均可被拒绝。" }], reviewTitle: "官方复核", review: ["所有说明须在 2026-11-20 AOE 前提交。", "允许多次提交；最高队内提交成为 Effective Final Submission。", "截止后进行统一验证、评分与获奖确认。"], ctaRules: "查看第二季 Rules", ctaSubmit: "前往 Kaggle 提交" },
  awards: { tag: "// 06 — 奖项", title: "第二赛季奖项", mainTitle: "现金奖", pool: { label: "第二赛季奖金池", amount: "$3,000", unit: "总计" }, main: [{ name: "Agent Master Builder", body: "最佳总体 verified Build with Agent 成绩。", badge: "主奖", prize: "$700" }, { name: "Best Build Autopilot", body: "最佳 verified Autopilot 成绩。", badge: "主奖", prize: "$500" }, { name: "Best Build Copilot", body: "最佳 verified Copilot 成绩。", badge: "主奖", prize: "$500" }], titlesTitle: "特别称号", titles: [{ name: "Powerful but Brutal", body: "强大但付出代价的机器。", prize: "$300" }, { name: "Cheap Thrills", body: "经济设计下的强劲表现。", prize: "$300" }, { name: "Most Creative Machine", body: "最具创意的机器或架构。", prize: "$300" }, { name: "Funniest Failure", body: "最令人记住的失败。", prize: "$200" }], communityTitle: "社区奖项", community: [{ name: "Community Favorite", prize: "$200" }, { name: "Best Human BOSS Build", prize: "称号" }, { name: "多个称号", prize: "允许" }, { name: "主现金奖", prize: "每机器一个" }], dedupeTitle: "奖项分配", dedupe: "一台机器可以获得多个展示称号，但最多获得一个主现金奖；资格或验证失败时按官方 roll-down 与复核规则处理。", academic: { title: "官方复核", badge: "Official", intro: "BuildArena 与指定评委在最终奖项前复核机器、控制器、telemetry、历史记录和说明。", contributionsTitle: "评委", contributions: ["BuildArena", "Nextan Pholizebort", "Nephoslbata"], adoptionNote: "奖项与资格以第二季官方 Rules 为准。" } },
  leaderboard: { tag: "// 07 — 排行榜", title: "第二赛季排行榜", status: "尚未开放", tabs: { overall: "总榜", autopilot: "Autopilot", copilot: "Copilot", human: "Human BOSS" }, columns: ["#", "队伍", "机器", "模式", "载荷", "效率", "回收", "着陆", "表现分", "最终分", "状态"], emptyTitle: "闭环正在预热", emptyBody: "比赛将于 2026-09-20 AOE 开始；统一评分后，提交 verified 最终任务即可展示。", provisional: "Provisional" },
